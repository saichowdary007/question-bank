    ...
    def _process_pdf(self, bucket: str, key: str) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                logging.info("📄 Extracted %d pages", len(pages)*3)
...
def _build_prompt(page_text: str) -> str:
    ...
    return f"""You are an intelligent educational assistant. Read the following page excerpt from a school book (Grades 1–10) and identify its main educational concepts. CONTENT VALIDATION: If the text is empty, contains only page numbers, headers, footers, navigation elements, or lacks substantive educational content (less than 30 meaningful words), return: []. If the text is fragmented, corrupted, or incomprehensible, return: []. Text must contain at least one clear educational concept with explanation. CONCEPT VERIFICATION: Before generating questions, check if the text contains: definitions, explanations, or descriptions of concepts; processes, functions, or cause-and-effect relationships; scientific facts or information with context. If no clear concepts are explained, return: []. QUESTION GENERATION RULES: Only if content contains clear educational concepts: 1. Generate as many meaningful multiple-choice questions as possible, aiming for at least 9 questions when content allows, maximum 15 questions. 2. Create questions from different aspects of each concept: definitions, applications, examples, processes, causes, effects, characteristics, functions. 3. Each question MUST be based on specific information explained in the text. 4. STRICTLY AVOID these question types and phrases: "What is mentioned in this text?", "What does this text discuss?", "According to the text...", "In this passage...", "In this text...", "In the text...", "In this...", "The text states...", "As described in the content...", "According to the figure shown...", "In the diagram...", "In this figure...", "In the fig...", "In this fig...", "The passage explains...", "Based on the reading...", "From the passage...", "The content shows...", "As shown above...", "As mentioned...", "This section discusses...", any reference to "text", "passage", "content", "figure", "diagram", "reading", "section", "above", "below", generic questions about page content or what is discussed. 5. Questions must be STANDALONE and self-contained without any reference to source material. 6. Frame questions as direct knowledge tests (e.g., "What causes...?", "How does...?", "Which process...?", "What is the definition of...?", "What happens when...?"). 7. Each question must have exactly 4 options labeled A-D with only one clearly correct answer. 8. Focus on testing understanding of key concepts, processes, or facts explained in the text. 9. If you cannot create at least 3 meaningful standalone questions from the content, return: []. 10. Randomize the position of the correct answer across A, B, C, D for each question to ensure an even distribution across all questions; avoid consistently placing correct answers as option A. 11. Verify that the correct answer is explicitly supported by the text; if any doubt exists about accuracy, skip that question. 12. Skip questions where the correct answer cannot be definitively determined from the provided text. OUTPUT FORMAT: Return ONLY valid JSON array format: [{{"question": "...", "options": ["A", "B", "C", "D"], "answer": "B"}}, ...]. Text: "{page_text}"""